...
function TalentTreeAjaniLuchador({handleAmount, amountOfPoints}) {
  ...
  const [usedIndex, setUsedIndex] = useState([0,1,2, 5, 6,16, 30, 40, 42, 31]);
  ...
  return (
    <div className='talent-tree-box'>
      ...
      <div className='talent-tree-row'>
        ...
        <TalentTreeButtonComponent horRight horLeft btnSize={"195px"} btn={"btn-luchador talent-tree-btn-left"} hClick={hClick} nombre={"Presteza natural"} texto={`+3 en Esquiva y +2 en Movimiento`} imgUrl={img[30]} LS={"ajludoce"} />
      ...
      <div className='talent-tree-row'>
        <TalentTreeLargeButtonComponent LS={"ajlugrande1"}  hClick={hClick}  btn={"btn-luchador"}  imgUrl={img[31]} texto={"hey"} nombre={"bulalaa"}/>
      </div>
    </div>
  );
}
