...
function TalentTreeButtonComponent({ points, nombre, vertical, horizontal, empty, btn, btnWidth, btnHeight, talentToSelect, filled }) {
  ...
  return (
    <>
      ...
      {horizontal &&
        <button style={{ width: `${btnWidth}`, height: `${btnHeight}` }}  className={ selected ? `talent-tree-btn-line-hor talent-tree-filled` : `talent-tree-btn-line-hor`}></button>
      }
      {vertical &&
        <button style={{ width: `${btnWidth}`, height: `${btnHeight}` }}  className={ selected ? `talent-tree-btn-line-ver talent-tree-filled` : `talent-tree-btn-line-ver`}></button>
      }
      {empty  && 
